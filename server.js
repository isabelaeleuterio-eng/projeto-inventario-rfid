...
app.post("/api/rfid/selecionar", async (req, res) => {
  try {
    ...
    if (!Number.isInteger(funcionarioId) || funcionarioId <= 0) {
      return res.status(400).json({ sucesso: false, erro: "Funcionário inválido." });
    }
    if (!Number.isInteger(equipamentoId) || equipamentoId <= 0) {
      return res.status(400).json({ sucesso: false, erro: "Equipamento inválido." });
    }
    ...
    if (acao === "retirada") {
      if (statusNormalizado(e.data.status) !== "disponivel") {
        return res.status(409).json({
          sucesso: false,
      ...
      if (possuiEmprestimo) {
        return res.status(409).json({
          sucesso: false,
          erro: "Esse equipamento já está emprestado para este funcionário."
        });
      }
    }
